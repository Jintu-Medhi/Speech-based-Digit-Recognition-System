// DigitRecog.cpp : Defines the entry point for the console application.
//
...
public:
		...
		for(int t = 1; t<=T-1; t++){
			for(int i = 1; i<=5; i++){
				for(int j = 1; j<=5; j++){
					...
					//temp = (temp <= 0) ? 1e-500 : temp;
					zai[t][i][j] = temp / probability ;
					//if(zai[t][i][j]<0) printf("%e\n ",zai[t][i][j]);
				}
			}
			//cout<<endl;
		}
	...
	void manipulateBmat(){
		for(int i = 1; i<=5; i++){
			for(int j = 1; j<=32; j++){
				b[i][j] = (b[i][j] <= LDBL_MIN) ? 1e-10 : b[i][j];
			}
		}
	}
	void findavg(int d){
...
void trainmodel(){
	...
	for(int d = 0; d<=9; d++){
		...
		for(int u = 1; u<=20; u++){
			string recordedFile = "./Recorded Files/234101022_E_" + to_string((long long) d) + "_" + to_string((long long) u) + ".txt";
			string cepstralFile = "./Cepstrals/234101022_E_" + to_string((long long) d) + "_" + to_string((long long) u) + ".txt";
			string vqFile = "./Observation Sequence/234101022_E_" + to_string((long long) d) + "_" + to_string((long long) u) + ".txt";
			string logfile = "./HMM logs/234101022_E_" + to_string((long long) d) + "_" + to_string((long long) u) + ".txt";
	...
	for(int i = 1; i<=5; i++){
		...
		for(int t = 1; t<=T-1; t++){
			for(int j = 1; j<=5; j++){
					long double temp = 0;
					for(int i = 1; i<=5; i++){
						temp += (alpha[t][i] * a[i][j] > 1e-500)? alpha[t][i] * a[i][j] : 1e-500   ;
					}
					alpha[t+1][j] = (temp * b[j][O[t+1]] > 1e-500)? temp * b[j][O[t+1]] : 1e-500 ;
					//printf("%Lf ",alpha[t+1][j]);//<<" ";
			}
			//cout<<endl;
		}
...
void testmodel(){
	...
	for(int d = 0; d<=9; d++){
		for(int u = 21; u<=30; u++){
			...
			string verdict = (maxi == d) ? " Correct" : "Wrong";
			accuracy += (maxi == d) ? 1 : 0; 
			cout<<"Digit recognized by HMM : "<<maxi<<verdict<<endl;
			cout<<"--------------------------------------------------------------------------------------------------------------------------------------------------------\n";
		}
	}
	cout<<"Accuracy of HMM Model : "<<accuracy<<"%"<<endl;	
}
int _tmain(int argc, _TCHAR* argv[])
{
	//trainmodel(); cout<<"Training completed successfully";		/* Dont run both train and test simultaneously */	
	testmodel();
